...
<?php  
	...
	if ( isset( $_POST['new_val_id_company_product'] ) ) {	
		//	Применяем функцию обнавления login
		if ( update_product_id_company() ) {
			// 	Проверка в консоли
			exit( "It is ok _ 3" );
		} else {
			exit( "Error _ 3" );
	...
	if ( isset( $_POST['new_val_id_category_product'] ) ) {	
		//	Применяем функцию обнавления login
		if ( update_product_id_category() ) {
			// 	Проверка в консоли
			exit( "It is ok _ 3" );
		} else {
			exit( "Error _ 3" );
	  ...
	  <tbody>
		...
		if ($result = mysqli_query( $link, 'SELECT * FROM products ORDER BY id' ) ) {
			 	...
			 	echo 	
			 		'<tr>' .
				 	 	...
				 	 	'<td><div class="edit_product_location" data-id="' . $row['id']. '" contenteditable>' . 
				 	 		$row['location'] . '</div></td>' .
				 	 	'<td>' . $row['date'] . '</td>' .
				 	 	'<td><div class="edit_id_company_product" data-id="' . $row['id']. '" contenteditable>' . 
				 	 		$row['id_company'] . '</div></td>' .
				 	 	'<td><div class="edit_id_category_product" data-id="' . $row['id']. '" contenteditable>' . 
				 	 		$row['id_category'] . '</div></td>' .
	    	...
	    	if ( isset( $_GET['del'] ) ) {
				...
				echo '<div class="alert alert-danger d-flex justify-content-center" role="alert"><h3>Вы успешно удалили товар с id' . $id  . '</h3></div>';
				//header("Refresh:0;  url=users.php");
				echo '<div class="text-center m-3"><a class="btn btn-warning" href="products.php">Close helper</a></div>';
